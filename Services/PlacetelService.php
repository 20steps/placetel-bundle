...
class PlacetelService {
    ...
    public function getService($id) {
    	if ($this->stopwatch) { $this->stopwatch->start('twentysteps\Bundle\Placetel\Services\PlacetelService.getService','20steps'); };
        ...
        if (false === ($service = $this->cache->fetch($cacheKey))) {
	        ...
	        if (array_key_exists('images',$service)) {
		        $service['images']['uptime4']='http://www.placetel.de/'.$service['images']['uptime4'];
		        $service['images']['latency4']='http://www.placetel.de/'.$service['images']['latency4'];	        	
	        }
	        $this->logger->info("Placetel returned ".var_export($service,true));
    ...
    public function getServices() {
    	if ($this->stopwatch) { $this->stopwatch->start('twentysteps\Bundle\PlacetelBundle\Services\PlacetelService.getServices','20steps'); };
        $call="service/list?apikey=".$this->apiKey;
		$this->logger->info('Calling PlacetelAPI: '.$call);
		try {
	        $request = $this->client->get($call,
	            array(), 
	            array(
